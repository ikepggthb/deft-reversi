...
## 使用した技術
 ...
 - 定石 book ([設計メモ](docs/book.md))
   - 評価値の上下界・探索の深さ・選択度を局面ごとに持つので、その値をどこまで
     信用してよいかが分かる。育成は幅の広いところから最良優先で掘る
   - 局面を手数の層に分けて持つので、値の伝播が層を 1 回舐めるだけで済み、
     読み込みも層ごとに並列化できる
   - 定石名を持つ。対局中に「この進行は 虎定石 です」と出せる
   - 標準は独自形式の `.dbk`。Egaroucid の `.egbk3` / `.egbk2` / `.egbk` と
     Edax の `book.dat` は取り込み・書き出しで対応
 - 機械学習(線形回帰)を用いた評価関数
