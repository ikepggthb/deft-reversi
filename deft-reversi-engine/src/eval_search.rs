...
pub fn negaalpha_eval_no_mo(board: &Board, mut alpha: i32, beta: i32, lv: i32, search: &mut Search) -> i32
{    
...
pub fn nws_eval_simple(board: &Board, alpha: i32, lv: i32, search: &mut Search) -> i32
{
    ...
    // move ordering
    if move_list_len >= 2 {
        set_move_eval(move_list, MOVE_ORDERING_EVAL_LEVEL_SIMPLE_SEARCH, search);  
        sort_move_list(move_list);
    }
...
{   
    ...
    if moves_bit == 0 { // 合法手がないならば
        let mut board = board.clone();
        board.swap(); //pass
        if board.moves() == 0 { // passしても合法手がない -> ゲーム終了
            board.swap();
            search.eval_search_leaf_node_count += 1;
            return solve_score(&board);
...
pub fn nws_eval(board: &Board, mut alpha: i32, lv: i32, search: &mut Search) -> i32
{
    ...
    if lv > 8 {
        if let Some(score) = et_cut_off(&mut alpha, &mut beta, move_list, lv, search.selectivity_lv, &mut n_skip, &search.t_table){
            return score;
        }
    }
    ...
    if tt_best_move != 0 {
        ...
        for move_board in move_list.iter_mut() {
            if move_board.put_place == move_str {
                ...
                if score >= beta {
                    search.t_table.add(board, score, SCORE_INF, lv, search.selectivity_lv, move_str);
                    return score;
                }
                if score > this_node_alpha {this_node_alpha = score;}
                if score > best_score {
    ...
    if move_list_len - n_skip >= 2 {
        set_move_eval(move_list, MOVE_ORDERING_EVAL_LEVEL, search);    
        sort_move_list(move_list);
    }
    ...
    for move_board in move_list.iter() {
        ...
        if score >= beta {
            search.t_table.add(board, score, SCORE_INF, lv, search.selectivity_lv, move_board.put_place);
            return score;
        }
        if score > this_node_alpha {this_node_alpha = score;}
        if score > best_score {
