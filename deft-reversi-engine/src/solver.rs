...
impl Solver {
    ...
    fn aspiration_search(&mut self, init_width: i32, predict_score: i32, solver: SolverType) -> i32 {
        ...
        loop {
            ...
            if (predict_score == -SCORE_MAX && alpha == -SCORE_MAX) ||  (predict_score == SCORE_MAX && beta == SCORE_MAX){
                break;
            }
    ...
    fn get_ai_rules(&self, board: &Board, lv: i32) -> (bool, i32, i32)
    {
        ...
        (
            ...
            if perfect_solver_use {std::cmp::min((n_empties - 7 - (2 - selectivity_lv_perfect_search * 2)).max(2) , lv)} else {lv} // perfect solver を使用する際は、反復深化でのEvalSolverレベルを制限
        )
    }
    ...
    pub fn solve(&mut self, board: &Board, lv: i32) -> SolverResult {
        ...
        self.candidate_boards = get_put_boards(board, legal_moves);
    ...
    fn eval_solver(&mut self, depth: i32, alpha: i32, beta: i32) -> i32
    {
        ...
        let mut best_score = -pvs_eval(&primary_board.board, -beta, -alpha, depth - 1, &mut self.search);
