...
`.egbk3` は 25 バイト固定長なので、読んだ塊をそのままレコード境界で切り分け
られる。デコードと正規化を **`available_parallelism()` 本のスレッドで並列に**
行い、表への登録だけを直列にする。
...
book を強化する処理 (`expand`) の時間はほぼ leaf の探索で、局面ごとの探索は
互いに独立している。1 局面の探索はたいてい浅く、探索の中を YBWC で割るより
**局面をばらまく方がはるかに効率が良い**。

- 共有カウンタで 1 件ずつ取らせる (work stealing)。局面によって探索時間が
  桁違いに変わるため、均等分割だと最も重い 1 件で律速される
- 探索中は book の表に触らないので同期が要らない。「探索する手の集合を作る」→
  「並列に探索」→「結果を反映」の 3 段に分けてある
- `Solver` は置換表がアトミックなので共有できる。共有していることは
  `src/search/solver.rs` のコンパイル時アサーションで保証している

level 16 で 12 巡 (1574 局面を追加):
...
CLI でも `book expand --threads 4` で 37.3 s → 9.6 s (3.89×)、結果は同一。
`--threads` は探索の中ではなく局面の並列度に配っている。

### 育成 1 巡あたりの計算量

以前の `expand` は 1 巡ごとに book 全体を走査して leaf の古くなった局面を
探していた。局面数 n に対して毎巡 O(n) かかり、大きな book では広げる数 k が
少なくても走査だけで時間を食う。

現在は前の巡で**変化した局面だけ**を追う。leaf が古くなるのは

- 展開した親 (leaf の手が book に入るため)
- 新しく追加した子 (まだ leaf が無いため)

に限られるので、この 2 つを次の巡の起点にすれば足りる。1 巡の手間が O(n) から
**O(k)** になった。book 全体の走査は最初の 1 回だけ。
...
`.egbk3` の読み込み (1000 万レコード、250 MB) の推移:

| | 時間 | ピーク RSS |
|---|---|---|
| 最初 (BTreeMap、全体読み) | 11.5 s | 684 MiB |
| ハッシュ表 + 正規化 1 回 | 4.4 s | 770 MiB |
| ストリーム読み | 2.8 s | 533 MiB |
| player 優先比較 | 2.1 s | 533 MiB |
| **デコードと正規化を並列化 (4 コア)** | **1.56 s** | 563 MiB |

残りは表への直列登録 (0.68 s) と I/O で、直列の下限に近い。
...
`expand` は最初に book 全体の leaf を整えたあと、次を繰り返す。

1. 前の巡で変化した局面の leaf を探索し直す (`search_leaves`、並列)
2. leaf の手の子局面を book に登録する
3. `negamax` — 評価値を親へ伝播させる

スレッドを使うなら `expand_with_threads(rounds, max_error, level, solver, threads)`。
`threads` は同時に探索する局面数で、探索の中の並列度ではない。
