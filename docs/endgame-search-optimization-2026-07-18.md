...
- CLIの`--threads N`はYBWC worker数を表す旧仕様である。`--threads 20`はmainを含めて最大21スレッド相当になる。
...
| 条件 | ノード数 | 時間 | NPS |
|---|---:|---:|---:|
| 提示されたYBWC再設計前、FFO40～59 | 43,932,950,623 | 65.792秒 | 667.749M |
...
| ルートYBWCを含む復元完了後、FFO40～59 | 40,254,491,756 | 62.900秒 | 639.971M |
...
4. `--threads`の意味を総スレッド数へ変更するか、CLI表示とドキュメントで明示する。
5. FFO40～59全体を固定CPU affinity、固定TTサイズ、同一ビルド条件で再計測する。
