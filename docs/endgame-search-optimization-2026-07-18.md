...
### Root boundとPV tableの実装結果

上記仮説を検証するため、以下を実装した。

- 同一selectivityのaspiration反復間で、ルート各手のlower/upper boundを着手番号ごとに保持する。
- 既知boundと次の探索窓を交差させ、完全値、fail-low、fail-highを次の反復で再利用する。
- main TTの1/16（最低1MiB）のPV tableを独立して確保する。
- 追加probe/storeコストを抑えるため、PV tableの対象はルートから4ply以内のPVSノードに限定する。
- PV復元ではPV tableをmain TTより優先する。

コミット`5da1c32`を基準として、別worktreeから同じnative条件でFFO57単独を1スレッド測定した。

| 実装 | ノード数 | 時間 | NPS |
|---|---:|---:|---:|
| `5da1c32` | 4,781,731,664 | 72.840秒 | 65.646M |
| Root bound + PV table | 4,738,611,595 | 73.116秒 | 64.809M |

ノード数は0.90%減ったが、時間は0.38%増で実質的に中立だった。trace上、aspiration再試行時の最初のルート子は約993Mノードから約949Mノードへ減ったが、空き30でEdaxとの差を説明する規模ではない。

20スレッドは探索順の揺れが大きく、5局面の単発比較では基準9.497B / 18.701秒に対して実装後10.791B / 20.292秒となった。FFO57単独も同じ実装で6.310B / 12.670秒から7.945B / 14.593秒まで変動した。PV tableを全PVS階層で試した段階の全FFO40～59測定は41.131B / 64.760秒で、全スコアが正解だった。現時点では並列性能の改善を示す再現性のある結果とはみなさない。

Root boundとPV tableは再探索を抑える基盤としては機能するが、次の主要対象はEdaxの段階的selectivity探索と、空き数に応じた手順評価の精度である。

### 段階的selectivityとTT手順再利用

空き27以上の完全読みに85%、95%、98%、99%、exactの段階探索を導入した。空き27～29ではEdaxのスケジュールに合わせ、99%段階を省略する。さらに深さ3以上の終盤手順評価では、候補の子局面がTTに存在する場合にボーナスを与え、異なるselectivityでcutoffに使えないTTも手順情報として再利用する。

FFO57、1スレッドの同一nativeビルドでの結果は以下となった。

| 実装 | ノード数 | 時間 | NPS |
|---|---:|---:|---:|
| Root bound + PV table | 約4.78B | 約72.8秒 | 約65.6M |
| 段階的selectivity | 2.446B | 60.040秒 | 40.737M |
| + TT子局面ボーナス | 2.349B | 57.860秒 | 40.600M |
| Edax | 1.350B | 22.169秒 | 60.884M |

段階探索は総ノードを約49%削減し、TT子局面ボーナスはさらに約4%削減した。一方、選択探索の評価ノードを含むためNPSは低下し、Edaxとの差は残る。

20スレッドのFFO57は3.234B / 27.298秒だった。`perf stat`のtask-clockは約90.7秒で、平均実効並列度は約3.34コアに留まった。空き28のFFO53は平均約9.3コアであり、空き30ではexactのaspiration再探索に残る最善ルート子の長い直列区間が並列効率を制限している。

### Local TTとPVS-YBWCの追加改善

- 13空き以下で使うthread-local TTを1層1024から2048 entryへ拡張した。
- local TTのindex計算を2乗算から、`player XOR rotate(opponent)`と1乗算のmixerへ変更した。
- `pvs_final`でもYoung Brotherを同期探索した後、残り兄弟のnull-window探索をworkerへ配るようにした。
- workerの結果がalphaを改善した場合だけ、親でPVS再探索する。

| 局面 | スレッド | 変更前 | 変更後 |
|---|---:|---:|---:|
| FFO53 | 1 | 約2.123B / 32.57秒 | 2.100B / 31.90秒 |
| FFO57 | 1 | 2.349B / 57.86秒 | 2.329B / 56.88秒 |
| FFO53 | 20 | 2.580B / 5.70秒 | 2.430B / 4.89秒 |
| FFO57 | 20 | 3.204B / 26.74秒 | 3.193B / 23.12秒 |

FFO57・20スレッドのtask-clockは約89.7秒で、平均実効並列度は約3.88コアへ改善した。Edaxとの並列性能差は残るが、PVS直列区間の分割は約13.6%の時間短縮になった。
...
- 終盤ordering深度を空き27以上で1段増やす案（FFO57が2.446B / 60.040秒から2.886B / 66.422秒へ悪化）
- exact移行時のaspiration窓を3～4へ広げる案（FFO57が約3.100B / 約70秒へ悪化）
- Edaxの98%/99%のz値2.6/3.3だけを移植する案（MPC回帰係数が異なり探索が大幅悪化）
- 空き30で99%段階を省略する案（予測値は同じでもTT手順準備が失われて大幅悪化）
- 深さ2以上のordering探索へfeature indexを再帰的に差分伝播する案（ノード不変で57.860秒から58.579秒へ悪化）
- Edax式の独立shallow TTと深さ5のordering探索（FFO57の手順・ノードがほぼ不変で約1.2%遅化）
- exact探索でlocal TTを無効化する案（NPSは約19%向上したがFFO53のノードが約49%増加）
- local TTを1層4096 entryへ拡張する案（ノードは減るが2048 entryよりキャッシュ負荷で遅化）
...
4. Edaxのように完全深さ到達後のselectivityを段階的に弱め、exact探索へ渡す予測値と手順を改善する。
5. potential mobility、edge stability、TT到達ボーナスを用いた適応的な手順評価を比較する。
6. 固定CPU affinityと複数回測定で、並列時の中央値とノード増加率を評価する。
