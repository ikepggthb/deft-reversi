...
- deft の速度向上比が 3.17 倍から 3.04 倍に下がっているが、これは
  **並列側の劣化ではない**。下の局所TT改善で 1 スレッドが 27.24 秒から
  25.1 秒に速くなり、分母が縮んだために比が小さく出ている。
  4 スレッドの実時間は 8.59 秒から 8.27 秒へ短縮している。
  速度向上比は 1 スレッドを速くするほど不利に出る指標なので、
  エンジン間の比較にも使いにくい。
