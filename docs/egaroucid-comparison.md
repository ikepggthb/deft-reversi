...
> **この節の数値は 2.10GHz 機のもので、後日 2.80GHz 機で取り直した結果とは
> 結論が違う。最新の測定は [ラウンドロビンによる再測定](#ラウンドロビンによる再測定)
> を見ること。**
...
上の表は「同一セッション内で連続して」測ったものだが、エンジンごとに
まとめて回していたため、測定中のマシン負荷の変動がエンジン間の差に
混ざっていた。また別セッション・別マシン (2.80GHz 機) の数値と
突き合わせようとして、比較にならないことが分かった。

そこで **5 エンジンを 1 ラウンドで 1 回ずつ回すラウンドロビン**で取り直した。

FFO #40-49、level 60、Intel Xeon 4コア **2.80GHz**、各 5 回の中央値。
deft は自身が出力する `total` の solve 時間 (評価関数 JSON の読み込みを含まない)。

| エンジン | 1スレッド | 全サンプル | 4スレッド | 全サンプル | 速度向上 | 1T ノード |
|---|---:|---|---:|---|---:|---:|
| deft `108fd1a` (ブランチ起点) | 33.22秒 | 32.95-36.87 | 13.08秒 | 12.56-13.68 | 2.54倍 | 796.5 M |
| deft `bd3998a` | 30.80秒 | 30.11-32.23 | 10.48秒 | 10.16-11.23 | 2.94倍 | 796.6 M |
| **deft `f8070c8` (現在)** | **30.78秒** | 30.62-31.66 | **10.48秒** | 10.01-10.71 | **2.94倍** | 796.6 M |
| edax 4.6 | **30.16秒** | 29.60-34.35 | **9.65秒** | 9.31-9.91 | 3.13倍 | 934.0 M |
| Egaroucid 7.8.0 | **29.24秒** | 29.01-30.87 | 9.81秒 | 8.33-11.15 | 2.98倍 | 676.3 M |

読み取れること。

- **ブランチ全体の効果**は 1 スレッドで 33.22→30.78 秒 (-7.3%)、
  4 スレッドで 13.08→10.48 秒 (**-19.9%**)。速度向上比は 2.54→2.94 倍。
  並列側の改善が主で、これは狙いどおり。
- `bd3998a` と `f8070c8` は 1T 30.80/30.78 秒、4T 10.48/10.48 秒で**完全に同じ**。
  1 スレッドのノード数も 796,638,645 で 1 ノードも違わない。
  レビュー指摘の修正 (分割点の共通化・テスト追加) は探索に影響していない。
- **2.10GHz 機での「deft は edax より速い」は、この機械では再現しない。**
  1T で edax が 2.0% 速く、4T では 8.7% 速い。4T は範囲が重ならない
  (edax 9.31-9.91 に対し deft 10.01-10.71) ので差は明確。
  1T の 2.0% は edax の範囲が広い (29.60-34.35) ため断定できない。
- Egaroucid は 1T で最速 (deft より 5.3% 速い) だが、4T では edax に抜かれ
  deft との差も 6.8% に縮む。4T の範囲が 8.33-11.15 と広く、
  並列時の再現性は 3 エンジン中もっとも悪い。
- 並列の伸びは edax 3.13 倍が最良で、deft 2.94 倍・Egaroucid 2.98 倍。
  **「並列化はもう遅れていない」は言い過ぎで、edax にはまだ届いていない。**

### 測定手順についての教訓

- **セッションをまたいだ時間の比較はできない。** 同じ FFO40-49 / level 60 で、
  2.10GHz 機の 25.1 秒に対し 2.80GHz 機では 30.8 秒だった。
  公称クロックが高いほうが遅いので、クロックからも換算できない。
  全エンジンが 12〜27% 遅くなっているが、遅くなり方はエンジンごとに違う
  (edax 1T が +11.7% に対し deft 1T は +22.7%) ため、
  **順位そのものが入れ替わる**。
- エンジンごとにまとめて回すのではなく、**ラウンドロビンで交互に回す**こと。
  この機械は負荷が変動し、単発では 6.4 秒の外れ値が出ることもある。
- スクリプトは `crossbench.sh` の形 (旧コミットは `git worktree` で
  チェックアウトしてそれぞれビルドし、同じ問題ファイルで回す) を使う。
...
> **注意**: 以下は 2.10GHz 機での測定にもとづく。2.80GHz 機のラウンドロビン
> 再測定では、1 スレッドでも 4 スレッドでも deft が 3 エンジン中もっとも遅く、
> 並列の伸びも edax に届いていない。「並列化は追いついた」は
> 機械依存の結論だったので、下の 1 行目は撤回する。
